...
from canids.types import ClassificationResults, Features
...
class DBConnector:
    def __init__(
        ...
        if not os.path.exists(db_path):
            if init_if_not_exists:
                ...
                raise ValueError(
                    f"Database path '{db_path}' does not exist. Init first with the migrate command."
                )
    ...
    def check_db_version(self, migrate_if_needed):
        ...
        if version != self.schema_version:
            logging.info(
                "Database schema has old version %s, must migrate to %s",
                version,
                self.schema_version,
            )
            if migrate_if_needed:
                ...
                logging.error(
                    "Run migrate command in order to migrate database schema. Exit."
                )
                sys.exit(1)
    ...
    def write_custom_model_table(self, model_id, table_name, content):
        table_cols = [
            (name, sqlite_type_of(type(value))) for name, value in content.items()
        ]
        ...
        with self.get_cursor() as c:
            values = [
                value if type(value) in [float, int, str, bool] else str(value)
    ...
    def update_hyperparameter_table(
        self, model_part_name: str, hyperparams: t.Dict[str, t.Any]
    ):
        ...
        with self.get_cursor() as c:
            for col_name, sqlite_type in must_add:
                logging.info(
                    "Add column %s (type: %s) to table %s",
                    col_name,
                    sqlite_type,
                    model_part_name,
                )
    ...
    def new_classification_info(self, classification_id, model_id):
        with self.get_cursor() as c:
            c.execute(
                "INSERT INTO classification_info (classification_id, model_id, is_finished) VALUES (?,?,0);",
    ...
    def save_classifications(self, r: ClassificationResults):
        with self.get_cursor() as c:
            data = [
                (r.classification_id, i, p.value)
                for i, p in zip(r.traffic_ids, r.predictions)
            ]
            c.executemany(
                "INSERT INTO classification_results (classification_id, record_id, label) VALUES (?,?,?);",
                data,
            )
    ...
    def get_classifications_records(self, classification_id: str) -> pd.DataFrame:
        with self.get_conn() as conn:
            df = pd.read_sql_query(
                "SELECT record_id, label FROM classification_results WHERE classification_id = ?",
    ...
    def get_all_classifications(self, with_count=False) -> pd.DataFrame:
        ...
        if with_count:
            sql = """
             ...
             """
        with self.get_conn() as conn:
            df = pd.read_sql_query(sql, con=conn, index_col="classification_id")
        return df
    ...
    def exist_features(self, fe_id: str, traffic_name: str):
        with self.get_conn() as conn:
            df = pd.read_sql_query(
                "SELECT fe_id FROM extracted_features "
                "WHERE fe_id = ? and traffic_name=?;",
                con=conn,
                params=(fe_id, traffic_name),
            )
        return len(df) > 0
    ...
    def load_features(self, fe_id, traffic_name) -> ndarray:
        with self.get_conn() as conn:
            df = pd.read_sql_query(
                "SELECT pickle_features FROM extracted_features "
                "WHERE fe_id = ? and traffic_name=?;",
                con=conn,
                params=(fe_id, traffic_name),
            )
        if len(df) == 0:
            raise ValueError(
                "Features for %s and %s do not exist in db!" % (fe_id, traffic_name)
            )
        obj = pickle.loads(df["pickle_features"][0])
        if type(obj) is not Features:
            raise ValueError(
                "Stored features in database have wrong type! Found %s" % type(obj)
            )
        return obj
    ...
    def load_model_by_fe_id(self, fe_id, traffic_name):
        with self.get_conn() as conn:
            df = pd.read_sql_query(
                "SELECT model_id FROM extracted_features "
                "WHERE fe_id = ? and traffic_name=?;",
                con=conn,
                params=(fe_id, traffic_name),
            )
    ...
    def migrate(self, old_version, new_version):
        ...
        with self.get_cursor() as c:
            if new_version >= 2 and old_version < 2:
                c.execute(
                    "ALTER TABLE classification_info ADD COLUMN is_finished INT DEFAULT -1;"
                )
                c.execute(
                    "ALTER TABLE classification_info  ADD COLUMN packet_count INT DEFAULT -1;"
                )
                c.execute(
                    "ALTER TABLE classification_info  ADD COLUMN packet_read_time FLOAT DEFAULT -1;"
                )
                c.execute(
                    "ALTER TABLE classification_info  ADD COLUMN total_time FLOAT DEFAULT -1;"
                )
                migrated = True
            if new_version >= 3 and old_version < 3:
                c.execute(
                    "ALTER TABLE evaluations ADD COLUMN is_aggregated INT DEFAULT 0;"
                )
        ...
        if migrated == False:
            raise ValueError(
                "Cannot migrate from database schema version %s to %s"
        ...
        logging.info(
            "Migrated from database schema version %s to %s"
            % (old_version, new_version)
        )
