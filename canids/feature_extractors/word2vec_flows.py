        ...
        for i in range(len(self.flows)):
            ...
            for packet_i, packet in enumerate(flow.packets):
                ...
                try:
                    ...
                    logging.error(
                        "Could not parse packet data for packet #%s of flow #%s: %s",
                        packet_i,
                        i,
                        e,
                    )
                ...
                words = [
                    ".".join(map(str, byte_group))
                    for byte_group in yield_groups(data, self.group_n_bytes)
                ]
                flow_words += words
            yield flow_words
...
class NetflowWord2Vec(BasicNetflowFeatureExtractor):
    def __init__(
        ...
        window_size: int = 10,
        min_count: int = 4,
        train_method=TrainingMethod.SKIP_GRAM,
        group_n_bytes=8,
        **kwargs,
    ):
    ...
    def _get_word2vec_features(self, sen_gen, length):
        ...
        for sentence in tqdm(sen_gen, total=length):
            word_vectors = [
                self.model[word] if word in self.model else self.model["not_found"]
    ...
    def init_parser(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--w2v-vector-size", type=int, default=50, dest="vector_size"
        )
        ...
        parser.add_argument(
            "--w2v-training-method",
            choices=[t.value for t in TrainingMethod],
            default=TrainingMethod.SKIP_GRAM,
            dest="train_method",
        )
        parser.add_argument(
            "--w2v-group-bytes", type=int, dest="group_bytes", default=8
        )

    @staticmethod
    def init_by_parsed(args: argparse.Namespace):
        return NetflowWord2Vec(
            vector_size=args.vector_size,
            train_method=TrainingMethod(args.train_method),
    ...
    def get_id(self) -> str:
        return (
            f"FlowWord2Vec({self.model}, training_sequence={self.training_name}, "
