...
class CLIParser:
    ...
    def _create_cli_parser(self):
        ...
        parser_train.add_argument(
            ...
            default=list(FEATURE_EXTRACTORS.keys())[0],
        ...
        parser_train.add_argument(
            ...
            type=str,
            dest="model_id",
            default=model_trainer.MODEL_ID_AUTO_GENERATE,
        )
        ...
        parser_classify = self._create_subparser(
            "classify",
            help="Feed traffic from a dataset and detect anomalies.",
        )
        parser_classify.add_argument(
            "--id",
            type=str,
            default=CLASSIFICATION_ID_AUTO_GENERATE,
        ...
        parser_evaluate = self._create_subparser(
            "evaluate",
            help="Generate an evaluation report in JSON format from a prediction log.",
        )
        ...
        parser_evaluate.add_argument(
            ...
            type=str,
            required=True,
            help="Id of the classification that will be evaluated.",
        )
        parser_evaluate.add_argument(
            "--force-overwrite",
            "-f",
        ...
        parser_evaluate.add_argument(
            ...
            nargs="+",
            dest="filter_names",
            help="Filters the traffic sequence names that are to be evaluated.",
        )
        ...
        parser_list_de.add_argument(
            "--short",
            "-s",
            help="Only list the names of the decision engines, without usage details",
            action="store_true",
        )

        parser_list_cl = self._create_subparser(
            "list-classifications",
            help="Lists all anomaly classifications that were previously run.",
        )
        parser_list_cl.add_argument(
            "--count",
            "-c",
            help="Additionally list the number of records for each classification.",
            action="store_true",
        )
        ...
        parser_list_models = self._create_subparser(
            "list-models", help="List available models."
        )
        parser_list_models.add_argument(
            "--model", "-m", default=None, help="Filter information by model id."
        )

        parser_list_evaluations = self._create_subparser(
            "list-evaluations", help="Prints evaluations."
        )
        parser_list_evaluations.add_argument(
            "--model", "-m", default=None, help="Filter evaluations by model"
        )
        parser_list_evaluations.add_argument(
            "--id", default=None, help="Filter evaluations by classification id"
        )
        ...
        hypertune = self._create_subparser(
            "hypertune",
            help="Hypertunes parameters of decision engine and feature extractor by running "
            "the train->classify->evaluate pipeline multiple times. ",
        )
        self._add_dataset_path_param(hypertune)
        hypertune.add_argument(
            "-f",
            "--file",
            type=str,
            required=True,
    ...
    def _add_dataset_path_param(self, subparser):
        subparser.add_argument(
            ...
            type=str,
            dest="dataset",
            default=list(DATASET_UTILS.keys())[0],
        ...
        subparser.add_argument(
            "--subset",
            dest="dataset_subset",
            help="Predefined subset of the dataset",
            default="default",
        )
    ...
    def _add_decision_engine_param(self, subparser):
        subparser.add_argument(
            "--decision-engine",
            type=str,
            dest="decision_engine",
            default=list(DECISION_ENGINES.keys())[0],
...
class CommandExecutor:
    ...
    def train(self, args: argparse.Namespace, unknown: t.Sequence[str]):
        ...
        feature_extractor, de, unknown, subparsers = resource_loaders.create_fe_and_de(
            de_name=args.decision_engine, fe_name=args.feature_extractor, args=unknown
        )
        ...
        trainer.start_training(
            store_features=args.store_features, load_features=args.load_features
        )
    ...
    def evaluate(self, args: argparse.Namespace, unknown: t.Sequence[str]):
        ...
        evaluator.evaluate(
            classification_id=args.id, filter_traffic_names=args.filter_names
        )
